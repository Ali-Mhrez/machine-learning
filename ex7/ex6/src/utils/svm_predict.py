...
def svm_predict(model, X, Xtrain, sigma):
  """
  ...
  if model.kernel.__name__ == "linear_kernel":
    ...
    # linear kernel
    p = X * model.w + model.b
  elif model.kernel.__name__ == "gaussian_kernel":
    ...
    for i in range(K_left.shape[1]):
        K[i, :] += K_left[0][i]
    K = model.kernel(np.array([1]), np.array([0]), sigma) ** K;
    K = model.y.T * K
    K = model.alphas.T * K
    p = np.sum(K, 1);
  else:
    ...
    for i in range(m):
      prediction = 0
      for ind, j in enumerate(model.support_.tolist()):
        prediction = prediction + model.dual_coef_[0,ind] * model.kernel(X[i, :].T, Xtrain[j, :].T);
      p[i] = prediction + model.intercept_;
