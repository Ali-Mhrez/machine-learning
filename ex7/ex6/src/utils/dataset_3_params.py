...
def dataset_3_params(X, y, Xval, yval):
  ...
  for c_candidate in candidates:
    for sigma_candidate in candidates:
      svm = SVC(C=c_candidate, kernel=lambda x1, x2: gaussian_kernel(x1, x2, sigma_candidate), tol=1e-3, max_iter=10000)
      model = svm.fit(X, y.ravel())
      
      predictions = svm_predict(model, Xval, X, sigma_candidate)
      error = np.mean(predictions != yval)
      if error < min_error:
        min_error = error
        c_opt = c_candidate
        sigma_opt = sigma_candidate
  # =========================================================================
  C = c_opt
  sigma = sigma_opt
  return (C, sigma)